...
donnees = np.loadtxt ('keff.csv', skiprows = 1, unpack = False, delimiter = ';')
timeVector = donnees[:,0]
tempVector = donnees[:,1:4]
tempVector = np.transpose(tempVector)
T1 = donnees[:,4]
...
nbThermo = len(tempVector)
nbPointsStudied = len(timeVector)
...
for thermo in range(nbThermo):
    temp = 1.0 - ((tempVector[thermo,:] - T0) / (T1-T0))
